 ...
 */
export function findVorbisSetup(sample, library) {
 ...
 */
export function decodeFmodVorbisSample(sample, library) {
    ...
    if (!setup) {
        ...
        const crc = vd && vd.length >= 4
            ...
            : -1;
        throw new Error(`FMOD Vorbis: no setup packet for CRC32 0x${crc >>> 0 ? crc.toString(16) : '<missing>'} (sample "${sample.name}"). The library covers ${library.count} known patterns; new ones turn up occasionally.`);
    }
    ...
    const audioPackets = [];
    const granules = []; // cumulative PCM samples after each packet
    {
        ...
        while (off + 2 <= data.length) {
            ...
            off += 2;
            if (packetSize === 0)
                break; // FMOD terminator
            if (off + packetSize > data.length)
                break;
...
// ---------------------------------------------------------------
function buildIdentificationPacket(channels, sampleRate) {
...
class OggBuilder {
    ...
    appendPage(packets, granulePos, bos, eos) {
        const segments = this.buildSegmentTable(packets);
        if (segments.length > MAX_SEGMENTS) {
            throw new Error(`Ogg page would have ${segments.length} segments (>${MAX_SEGMENTS}); packet too large`);
        }
        ...
        for (let i = 0; i < segments.length; i++)
            page[HEADER_BYTES + i] = segments[i];
        let off = headerSize;
    ...
    buildSegmentTable(packets) {
        const segs = [];
        for (const pkt of packets) {
            const full = Math.floor(pkt.length / SEGMENT_SIZE);
            for (let i = 0; i < full; i++)
                segs.push(SEGMENT_SIZE);
            segs.push(pkt.length % SEGMENT_SIZE);
        }
        return segs;
    }
}
//# sourceMappingURL=decode-vorbis.js.map