 ...
 */
export { parseFsb5, isFsb5, SOUND_FORMAT, SOUND_FORMAT_NAMES, METADATA_CHUNK_TYPE, FREQUENCY_VALUES, } from './parse.js';
export { decodeSampleToPcm16, encodeWavBlob, } from './decode-pcm.js';
export { FmodVorbisSetupPackets, loadFmodVorbisSetupPackets, } from './setup-packets.js';
export { decodeFmodVorbisSample, findVorbisSetup, } from './decode-vorbis.js';
 ...
 */
export async function decodeSampleToBlob(sample, mode, vorbisLibrary) {
    switch (mode) {
        ...
        case SOUND_FORMAT.VORBIS: {
            if (!vorbisLibrary) {
                throw new Error(`FSB5 Vorbis playback needs the setup-packet library. Pass \`vorbisLibrary\` to decodeSampleToBlob — load \`assets/fmod_vorbis_setup_packets.bin\` from @tootallnate/fsb5.`);
            }
        ...
        default:
            throw new Error(`FSB5 codec ${mode} (${SOUND_FORMAT[mode] ?? `mode_${mode}`}) is not supported for browser playback yet.`);
    }
}
